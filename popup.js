...
startButton.addEventListener("click", async () => {
  ...
  statusText.textContent = isFullPage
    ? "Capturing full page..."
...
function detectTopColors() {
  ...
  for (let i = 0; i < imageData.length; i += 4) {
    ...
    for (let [existingColorKey, count] of colorMap.entries()) {
      const [existingR, existingG, existingB] = existingColorKey
        .split(",")
        .map(Number);
...
function drawCanvas(images) {
  ...
  const isFullPage = captureModeToggle.checked;
  statusText.textContent = isFullPage
    ? "Full page capture complete"
    : "Visible area capture complete";

  // Reset top colors section
  document.getElementById("loadingColors").textContent =
...
analyzeButton.addEventListener("click", () => {
  ...
  const results = counts.map((count, index) => {
    const hex = rgbToHex(
      selectedColors[index][0],
      selectedColors[index][1],
      selectedColors[index][2]
    );
    const percentage = ((count / totalAnalyzedPixels) * 100).toFixed(2);
    return {
      hex: hex,
...
function displayAnalysisResults(results) {
  // Update chart colors
  document.getElementById("chartColor1").style.backgroundColor =
    results.color1.hex;
  document.getElementById("chartColor2").style.backgroundColor =
    results.color2.hex;
  document.getElementById("chartColor3").style.backgroundColor =
    results.color3.hex;

  // Update chart labels
  document.getElementById(
    "chartLabel1"
  ).textContent = `Color 1 (${results.color1.hex})`;
  document.getElementById(
    "chartLabel2"
  ).textContent = `Color 2 (${results.color2.hex})`;
  document.getElementById(
    "chartLabel3"
  ).textContent = `Color 3 (${results.color3.hex})`;

  // Update percentages
  document.getElementById("chartPercentage1").textContent =
    results.color1.percentage;
  document.getElementById("chartPercentage2").textContent =
    results.color2.percentage;
  document.getElementById("chartPercentage3").textContent =
    results.color3.percentage;

  // Update chart fills with animation
  setTimeout(() => {
    document.getElementById("chartFill1").style.width =
      results.color1.percentage;
    document.getElementById("chartFill2").style.width =
      results.color2.percentage;
    document.getElementById("chartFill3").style.width =
      results.color3.percentage;
  }, 100);

  // Update summary
  document.getElementById("totalPixels").textContent = results.totalPixels;
  document.getElementById("captureModeSummary").textContent =
    results.captureMode;
